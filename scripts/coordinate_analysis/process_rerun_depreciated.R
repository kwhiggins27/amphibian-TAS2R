...
setwd("/Volumes/wengpj01/vertebrate_pipeline/20231006_run")

api_key <- Sys.getenv("CHANGE THIS")
genes_accessions_input <- "/Volumes/wengpj01/vertebrate_pipeline/20231006_run/20231006_basics.csv"
clusters_accessions_input <- "/Volumes/wengpj01/vertebrate_pipeline/20231006_run/summary_clusters_withnearest_1000000.csv"
genome_info_input <- "/Volumes/wengpj01/genomes/accessions_pt1_assembly_combo.csv"
position_coordinates_input <- "/Volumes/wengpj01/vertebrate_pipeline/coordinates/position_within_chromosome_1005.csv"
genome_size_input <- "/Volumes/wengpj01/vertebrate_pipeline/genome_size.csv"
...
# taxonomic_info_2 <- sapply(merged_main_no_duplicates$latin, get_taxonomic_info)
taxonomic_info_2 <- read_csv("tax_info_RM_batch.csv")
...
output_file <- "match_accession_class.csv"
...
graphics.off()
plotTree.barplot(taxize_tree,matrix_data,args.barplot=list(col="blue"))



#Build a tree
duplicates <- merged_with_taxa[duplicated(merged_with_taxa$taxid), ]
taxize_class <- classification(merged_with_taxa$taxid, db = "ncbi")
taxize_tree <- class2tree(taxize_class, check = TRUE)
# tree <- plot(taxize_tree)

pdf("genes_with_tree.pdf")
#dotTree(tree,matrix_data,length=10,method="plotTree", standardize=FALSE)
plotTree.barplot(taxize_tree$phylo,matrix_data,args.barplot=list(col="blue"))
dev.off()
