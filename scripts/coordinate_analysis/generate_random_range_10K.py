 ...
 #        return
    ...
    for index, row in chromsizes.iterrows():
        ...
        if length > 10000:
            ...
            if largest_start < running_total_length - 10000:
                largest_start = running_total_length - 10000
    ...
    #         if not j.empty and (j["length"].values[0] - 10000 >= (i - j["after"]).values[0]):
    #             random_starts.append(i)
    #             break
    #
    for _ in range(100):
        while True:
            i = random.randint(0, running_total_length-10000)
            j = chromsizes[(chromsizes["before"] < i) & (chromsizes["after"] >= i)]
            # Initialize k
            k = None
            ...
            if not j.empty:
                ...
                if (j["length"].values[0] - 10000) >= (i - j["before"].values[0]):
                    random_starts.append(i)
