...
data_dr <- read_csv("../../results/assay/data_DRcurve_ver240516.csv")
...
data_dr$gene <-  str_replace_all(data_dr$gene, name_convert)
...
plot_limit_y <- tibble(
  ...
  gene = c("axolotl_54",
           ...
           "clawed_23"),
  group = "DRcurve",
  AUC = c(6e+4, 0.9e+4, 7e+4, 1.5e+4, 
          4e+4, 0.9e+4, 0.9e+4, 2.5e+4),
  file = "empty",
   )
...
line_DR <- data_dr_plot %>%
  ggplot(aes(x = concentration, y = AUCx4, colour = compound, fill = compound, shape = compound, linetype = compound)) +
